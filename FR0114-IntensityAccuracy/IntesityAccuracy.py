...
def findXML(col,row,hand):
    if hand == 'left':
        tree = ET.parse(im_LXML)
    else:
        tree = ET.parse(im_RXML)
        
    root    = tree.getroot()
    XCam    = -1;
    YCam    = -1;
    
    for child in root.iter('Coordinate'):
        colVal = int(child.get('Column'))
        rowVal = int(child.get('Row'))
        XVal   = int(child.get('X'))
        YVal   = int(child.get('Y'))
...
im_LXML       = 'C:\\Users\\GWA\\Desktop\\Internship DEMCON\\4. Data Acquisition\\3. IntensityAccuracy [FR0114]\\source\\left-motormatrix.xml'
im_RXML       = 'C:\\Users\\GWA\\Desktop\\Internship DEMCON\\4. Data Acquisition\\3. IntensityAccuracy [FR0114]\\source\\right-motormatrix.xml'
...
for colind in range (0,grid_size[0]):                 # mask grid column index
    for rowind in range (0,grid_size[1]):             # mask grid row index
        ...
        colL = offsetL[0] + cell_size[0]*(colind)
        col_coordL.append(colL)
        colR = offsetR[0] + cell_size[0]*(colind)
        col_coordR.append(colR)
        
        # left and right ROW
        rowL = offsetL[1] + cell_size[1]*(rowind)
        row_coordL.append(rowL)
        rowR = offsetR[1] + cell_size[1]*(rowind)
        row_coordR.append(rowR)
         
maskL_coord = np.array([col_coordL,row_coordL])
maskL_coord = np.transpose(maskL_coord)

maskR_coord = np.array([col_coordR,row_coordR])
maskR_coord = np.transpose(maskR_coord)

## Camera Coordinate
# initialization
camR_coord = [0,0]
camL_coord = [0,0]
# processing camera coordinate
for i in range (0, len(maskR_coord)):
    xL_coord,yL_coord = findXML(maskL_coord[i,0],maskL_coord[i,1],'left')
    camL_coord = np.vstack((camL_coord,[xL_coord,yL_coord]))
    xR_coord,yR_coord = findXML(maskR_coord[i,0],maskR_coord[i,1],'right')
    camR_coord = np.vstack((camR_coord,[xR_coord,yR_coord]))

camL_coord = camL_coord[1:]
camR_coord = camR_coord[1:]   

## Location the coordinate
a  = img.imread(im_01_RawLnir)
